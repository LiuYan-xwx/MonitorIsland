...
namespace MonitorIsland.Abstractions.Providers;
...
public abstract class MonitorProviderBase : IDisposable
{
    ...
    public static MonitorProviderBase? GetInstance(MonitorProvider? monitorProvider)
    {
        ...
        var provider = IAppHost.Host?.Services
            .GetKeyedService<MonitorProviderBase>(monitorProvider.Id);
        if (provider is null)
            return null;

        var settingsType = provider.GetType().BaseType?
            .GetGenericArguments()
            .FirstOrDefault();
...
public abstract class MonitorProviderBase<TSettings> : MonitorProviderBase
